...
post:likes:{postId}:init     "1"             bootstrap 완료 마커 (SETNX로만 세팅)
```
...
local key = KEYS[1]
local member = ARGV[1]
local lastResult = ARGV[2]   -- "1" (좋아요 후 실패) or "0" (취소 후 실패)

if lastResult == '1' then
    redis.call('SREM', key, member)   -- 좋아요 후 실패 → 취소
else
    redis.call('SADD', key, member)   -- 취소 후 실패 → 복구
end
...
[PostLikeService.like()]
        ...
        │     compensateLua(postId, memberId, liked)  보상 Lua
...
2. SET post:likes:{postId}:init "1" NX EX 60      ← SETNX, 60s 임시 임대
       └─ false (이미 다른 스레드가 bootstrap 중) → 짧게 spin or skip
       └─ true (락 획득)
3. DB: SELECT member_id FROM post_like WHERE post_id=? AND deleted=false  (PAGE 단위)
4. SADD post:likes:{postId} memberId...  (배치)
5. PERSIST post:likes:{postId}:init   (영구 마커로 전환, 또는 다시 SET "1"로 TTL 제거)
```

**락 임대 시간 60초**는 bootstrap이 끝나기 전 init 키가 누수되지 않도록 안전망. 정상 흐름에선 5단계에서 PERSIST로 영구화한다.
...
- [ ] 30 스레드 동시 토글에서 `SCARD == post_like COUNT`
- [ ] Redis 컨테이너 stop 후에도 좋아요 API가 5xx 없이 응답
- [ ] DB INSERT 실패 mock 시 Redis 상태가 보상 Lua로 원복
- [ ] `post.like_count` 컬럼 및 관련 코드가 schema/엔티티/서비스 어디에도 없음
- [ ] Lua script가 SHA1로 캐싱되어 `EVALSHA`로 실행됨 (Spring 로그 확인)
