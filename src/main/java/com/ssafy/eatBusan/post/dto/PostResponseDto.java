...
public record PostResponseDto(Long postId, Long userId, Long placeId, String email, String title, String content,
                              long viewCount, long likeCount, int commentCount, boolean liked,
                              LocalDateTime createdAt, LocalDateTime updatedAt, List<PostImageDto> images) {
    ...
    public static PostResponseDto from(Post post, long likeCount, List<PostImageDto> images) {
        return from(post, likeCount, images, false);
    }

    public static PostResponseDto from(Post post, long likeCount, List<PostImageDto> images, boolean liked) {
        return new PostResponseDto(post.getId(), post.getMember().getId(), post.getPlace().getId(),
            ...
            likeCount, post.getCommentCount(), liked, post.getCreatedAt(), post.getUpdatedAt(), images);
    }
}
