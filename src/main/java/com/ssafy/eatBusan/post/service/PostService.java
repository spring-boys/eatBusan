...
public class PostService {
    ...
    public List<PostResponseDto> getAllPost() {
        return postRepository.findAllByDeletedFalse().stream()
                .map(p -> new PostResponseDto(
                        ...
                        p.getViewCount(), postLikeCacheService.likeCount(p.getId()), p.getCommentCount(),
    ...
    public PostResponseDto writePost(PostRequireDto req) {
        ...
        Place place = placeRepository.findById(req.placeId()).orElseThrow(() -> new EBException(ErrorCode.PLACE_NOT_FOUND));
        Post post = Post.builder().member(member).place(place).title(req.title()).content(req.content()).build();
        postRepository.save(post);
        return PostResponseDto.from(post, postLikeCacheService.likeCount(post.getId()));
    }
